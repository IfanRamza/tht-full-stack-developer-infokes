...
async function seed() {
  ...
  try {
    ...
    const coreRoots: NewItem[] = [
      {
        id: rootIds.documents,
        name: "Documents",
        type: "folder",
        parentId: null,
        path: `/${rootIds.documents}`,
        depth: 0,
        sortOrder: 0,
      },
      {
        ...
        type: "folder",
        parentId: null,
        path: `/${rootIds.pictures}`,
        depth: 0,
        sortOrder: 1,
      },
      {
        id: rootIds.downloads,
        name: "Downloads",
        type: "folder",
        parentId: null,
        path: `/${rootIds.downloads}`,
        depth: 0,
        sortOrder: 2,
      },
      {
    ...
    const docFiles: NewItem[] = [
      {
        ...
        name: "Work",
        type: "folder",
        parentId: rootIds.documents,
        path: `${docPath}/${docIds.work}`,
        depth: 1,
        sortOrder: 0,
      },
      {
    ...
    await db.insert(items).values([
      ...
      },
      {
        ...
        name: "Frontend_Vue",
        type: "folder",
        parentId: engFolderId,
        path: `${engPath}/${frontendFolderId}`,
        depth: 3,
        sortOrder: 0,
      },
      {
    ...
    const frontendAssets = generateRandomItems(
      frontendFolderId,
      `${engPath}/${frontendFolderId}`,
      3,
      75,
    );
    const backendAssets = generateRandomItems(
      backendFolderId,
      `${engPath}/${backendFolderId}`,
      3,
      75,
    );
    ...
    const downloadDump = generateRandomItems(
      rootIds.downloads,
      `/${rootIds.downloads}`,
      0,
      60,
    ...
    console.log(
      `✅ Seeding complete — Inserted ~240 highly varied items across the tree.`,
    );
  } catch (error) {
